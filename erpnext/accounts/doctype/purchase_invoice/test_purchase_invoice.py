...
class TestPurchaseInvoice(FrappeTestCase, StockTestMixin):
	...
	def test_partly_paid_of_pi_to_pr_to_pe_TC_B_081(self):
		...
		paid_to_account = frappe.db.get_value(
			"Account",
			{"company": "_Test Company", "account_type": ["in", ["Payable"]], "is_group": 0},
			["name"],
		)
		get_or_create_fiscal_year("_Test Company")
	...
	def test_validate_available_budget_codecov(self):
	...
	@if_app_installed("projects")
	def test_update_actual_overall_budget_codecov(self):
		project_name = "test_project" + frappe.generate_hash(length=5)
		if not frappe.db.exists("Project", {"project_name": project_name}):
			frappe.get_doc(
				{"doctype": "Project", "company": "_Test Company", "project_name": project_name, "is_wbs": 1}
	...
	@if_app_installed("projects")
	def test_update_locked_actual_overall_budgets_codecov(self):
		project_name = "test_project" + frappe.generate_hash(length=5)
		if not frappe.db.exists("Project", {"project_name": project_name}):
			frappe.get_doc(
				{"doctype": "Project", "company": "_Test Company", "project_name": project_name, "is_wbs": 1}
	...
	@if_app_installed("projects")
	def test_locked_committed_overall_budget_po_to_pi_codecov(self):
		from erpnext.buying.doctype.purchase_order.purchase_order import make_purchase_invoice
		from erpnext.buying.doctype.purchase_order.test_purchase_order import create_purchase_order
